...
if (!empty($pagina)) {
    ...
    $query_usuarios = "SELECT usr.id, usr.nome, usr.email,
                      ...
                      LEFT JOIN enderecos AS ende ON ende.usuario_id=usr.id ORDER BY usr.id DESC LIMIT $inicio, $qnt_result_pg";
    $result_usuarios = $conn->prepare($query_usuarios);
    $result_usuarios->execute();

    //Verificar se há registros a se a consulta foi bem-sucedida
    if (($result_usuarios) and ($result_usuarios->rowCount() != 0)) {
        ...
        //Paginação - Somar a quantidade de registros que há BD
        ...
        $quantidade_pg = ceil($row_pg['num/result'] / $qnt_result_pg);

        $max_links = 2;

        //Construir a navegação da pagina
...
//Um arquivo JSON(JavaScript Object Notation) é um formato leve de troca de dados que utilize uma sintaxe fácil de ler e escrever.Ele é frequentemente usado para representar estruturas de dados simples, como pares chave-valor e objetos, e é comumente usado em comunicações entre servidores web e clientes, sendo fácil de entender tanto por humanos quanto por máquinas
